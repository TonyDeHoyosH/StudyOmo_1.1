// --- FUNCIÓN AUXILIAR PARA FORMATEAR FECHA Y HORA DE UN ARREGLO ---
/**
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Enlazamos el botón de toggle (volver a pendientes)
  const linkPendientes = document.getElementById("linkPendientes");
  if (linkPendientes) {
    linkPendientes.href = `./tareasNoCompletadas.html?idGrupo=${idGrupo}&rol=${rol}`;
  }

  try {
    // Usando el endpoint correcto que proporcionaste
    const response = await fetch(`http://100.29.28.174:7000/usuarios/${idUsuario}/grupos/${idGrupo}/tareas-completadas`);
    ...
    if (response.ok) {
      ...
      if (tareas.length === 0) {
        contenedor.innerHTML = `<p style="color: white; text-align: center;">Aún no has completado ninguna tarea en este grupo.</p>`;
        return;
      }
      ...
      tareas.forEach(tarea => {
        ...
        // Usamos la función para formatear la fecha antes de mostrarla.
        tareaItem.innerHTML = `
          ...
          <span class="class-item__objective">Entregada el: ${formatLocalDateTime(tarea.fechaEnvio)}</span>
      ...
      alert("No se pudieron cargar las tareas completadas. Estado: " + response.status);
    }
...
function verTarea(idTarea) {
  ...
  window.location.href = `./verEvidencia.html?idTarea=${idTarea}&idGrupo=${idGrupo}&rol=${rol}`;
}